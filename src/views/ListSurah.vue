<template>
    ...
    <input type="text" v-model="searchSurah" @keyup="searchOfSurah" class="border w-full h-10 px-2 focus:outline-none rounded mt-2 ring ring-blue-500 shadow-sm" placeholder="Ketikkan Nama Surah">
        <div v-if="resultSearch.list.length > 0">
            ...
            <div v-for="(list, index) in resultSearch.list" :key="index">
                <!-- <router-link :to="{name: 'DetailSurah', params: {id: list.number} }" class="hvrlist"> -->
                    <a @click="detailSurah(list.number)" class="hvrlist">
                        <CardSurah :data="list" />
                    </a>
                <!-- </router-link> -->
            </div>
        ...
        <div v-else>
            <div class="grid gap-4 grid-cols-1 lg:grid-cols-3 mt-5">
                <div v-for="(list, index) in quran.list" :key="index">
                    <!-- <router-link :to="{name: 'DetailSurah', params: {id: list.number} }" class="hvrlist"> -->
                        <a href="#" @click="detailSurah(list.number)" class="hvrlist">
                            <CardSurah :data="list" />
                        </a>
                    <!-- </router-link> -->
                </div>
